...
<body class="d-flex flex-column min-vh-100">
      ...
      <div class="row justify-content-center">
        <div class="col-lg-6">
          ...
          <div class="card shadow-sm p-4">
            ...
            <button id="pdfButton" class="btn btnEditar w-100 fs-5 mt-4" disabled>
  ...
  <!-- Modal de Cargando (muestra mientras se genera/descarga el PDF) -->
  <div class="modal fade" id="loadingModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content text-center align-items-center justify-content-center p-4">
        <div class="spinner-border color1 mb-3" role="status">
          <span class="visually-hidden">Cargando...</span>
        </div>
        <h5 class="mb-0">Generando reporte...</h5>
      </div>
    </div>
  </div>
...
// Función para descargar blob y forzar descarga
...
fechaInput.addEventListener("change", function() {
  ...
  if (fecha) {
    ...
    pdfButton.onclick = async (e) => {
      ...
      try {
        // Llamada al endpoint que genera el PDF
        const res = await fetch(`/reporte/asistencia/pdf?fecha=${encodeURIComponent(fecha)}`, {
          method: 'GET',
          headers: { 'Accept': 'application/pdf' }
        });

        if (!res.ok) {
          throw new Error('Error al generar el PDF (código ' + res.status + ')');
        }

        const blob = await res.blob();
        const filename = `reporte_asistencia_${fecha}.pdf`;
        ...
        setTimeout(() => {
          loadingModal.hide();
        }, 500); // 0.5 segundos, ajustable si quieres más tiempo
      } catch (err) {
        console.error(err);
